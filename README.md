...
| `syncpct` | 40 | 1手の思考予算のうち信念の同期・再生成に回す割合(%) |
| `blockcp` | 0 | 相手の反則を誘う配置への加点(0で無効。未較正) |
| `oppmodel` | 0 | 探索の相手ノードを非千里眼モデルにする(1=直後の応手だけ / 2=全相手ノード)。**既定オフ**: 240局の測定で勝率は五分だった(下記) |
...
直近の実測でわかったこと(240局、docs/strengthening.md 1.9章):
**探索の相手ノードから千里眼バイアスを取り除くと打ち方は劇的に変わる**
(盤上で決着する局が18% → 32.5%と約2倍、`king_acc` も 13.1% → 17.0%)が、
**勝率は49.6%と1ポイントも動かない**。設計時に「次の律速」と見込んだ場所は外れで、
