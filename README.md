...
現状の実測: 前進ヒューリスティック(サイト内蔵bot相当)に **20戦全勝**(反則0.4回/局。初版は2.2回/局)。

初版と同じ設定(`opppolicy 0 deduce 0 synthprior 0 foulbase 100`)との直接対決は
60局で **35勝25敗**。1手あたりの反則は **10.0% → 8.0%** と明確に減っている(z=3.4)が、
**勝率58%は60局でもまだ有意ではない**。勝ち星35のうち31は反則の消耗戦由来で、
盤上の決着は4勝5敗のほぼ互角。信念を良くしても「勝ちにいく」力は動いていないので、
次の律速は探索側の千里眼バイアス。詳細は docs/design.md の 4.5 / 5 章。
