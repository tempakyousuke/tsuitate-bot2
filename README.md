...
現状の実測: 前進ヒューリスティック(サイト内蔵bot相当)に **20戦全勝**(反則0.25回/局。初版は2.2回/局)。

初版と同じ設定(`opppolicy 0 deduce 0 synthprior 0 foulbase 100`)との直接対決は
60局で **32勝28敗**。1手あたりの反則は **9.8% → 7.2%** と明確に減っている(z=4.6)一方、
**勝率は53%で有意差なし**。決着理由で分けると反則負けは28勝19敗と勝ち越すのに、
詰みは4勝9敗で負け越しており、信念の改善を盤上の勝ちに変換できていない。
