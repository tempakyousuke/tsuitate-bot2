...
| `foulbase` / `foulstep` | 800 / 60 | 反則の期待コストと、反則累計1回ごとの追加ぶん |
...
現状の実測: 前進ヒューリスティック(サイト内蔵bot相当)に **20戦全勝**(反則0.95回/局)。
初版の設定(`opppolicy 0 deduce 0 foulbase 100`)との直接対決は **11勝1分4敗**。
詳細と次の一手は docs/design.md の 4.5 / 5 章。
