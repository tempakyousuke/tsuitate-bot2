...
| `oppmodel` | 0 | 探索の相手ノードを非千里眼モデルにする(1=直後の応手だけ / 2=全相手ノード)。**既定オフ**: 233局で勝率54.1%(有意でない)、採用ゲート未達(下記) |
...
直近の実測でわかったこと(233局、docs/strengthening.md 1.11章):
**探索の相手ノードから千里眼バイアスを取り除くと打ち方は大きく変わる** ——
盤上で決着する局が 18% → 35.6% と約2倍になり、1局あたりの反則も有意に減る
(対応のあるt検定で t=−3.30)。**しかし勝率は54.1%(有意でない)、
盤上の勝敗は42勝41敗で互角**で、採用ゲートの60%には遠い。
