...
現状の実測: 前進ヒューリスティック(サイト内蔵bot相当)に **20戦全勝**(反則0.15回/局。初版は2.2回/局)。

初版と同じ設定(`opppolicy 0 deduce 0 synthprior 0 foulbase 100`)との直接対決は
60局で **36勝24敗**。1手あたりの反則は **9.1% → 7.2%** と明確に減っている(z=3.3)が、
**勝率60%は60局でもまだ有意ではない**。勝ち星36のうち30は反則の消耗戦由来で、
盤上で決着する局は60局中10局しかない。信念を良くしても「勝ちにいく」力は
動いていないので、次の律速は探索側の千里眼バイアス。詳細は docs/design.md の 4.5 / 5 章。
