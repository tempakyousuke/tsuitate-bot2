...
struct Config {
	...
	//   「ジョブ数 × その深さの1ジョブあたり実測wall時間(対局内で学習するEMA)」で
	//   予測し、締め切りまでに収まるなら次のパスを始める。その深さの実測がまだ
	//   無ければ直前のパスの所要時間 × passGrowth(halving なら候補数の縮小比も
	//   掛ける)で代用する。最初のパス(d=2)が丸ごと収まらないときは粒子数を
	//   収まる範囲に減らして読む(深さ0で終わるよりは浅い値でも全候補に付ける)。
	//   始めたパスが間に合わなければ従来どおり破棄。
	int    passGate   = 0;
