...
struct Config {
	...
	// 千里眼の最善応手(相手視点のmax)を混ぜる割合。
	// 純粋な盲目モデルは逆方向に楽観的すぎる(実際の相手は自分なりの信念で
	// こちらの駒を推測してくる)ので、min/max混合で「部分的に見えている相手」を
	// 粗く近似する。0=完全に盲目 / 1=従来の千里眼と同じ。
	double oppLambda    = 0.25;
	// 相手の反則1回をこちらの利得としてどれだけ評価するか(foul_value への倍率)。
	// 相手の反則はこちらの信念を壊さないぶん、自分の反則より安い可能性がある。
	double foulGainScale = 1.0;
